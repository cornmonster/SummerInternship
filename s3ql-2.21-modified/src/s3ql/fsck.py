...
class ROFsck(Fsck):
	...
	# replace the last '/' with '/../'
        ...
        temp_path = path[0:idx_last_slash] + '/..' + path[idx_last_slash:]
...
def main(args=None):
    ...
    if os.path.exists(cachepath + '.params'):
        assert os.path.exists(temp_cachepath + '.db')
    ...
    if db:
        ...
        try:
            ...
            raise QuietError('Local metadata is corrupted. Remove or repair the following '
                             ...
                             + temp_cachepath + '.db (corrupted)\n'
                             + cachepath + '.param (intact)', exitcode=43)
    else:
        db = download_metadata(backend, temp_cachepath + '.db')
